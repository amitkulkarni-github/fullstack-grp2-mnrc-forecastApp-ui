...
export class SalesForecastComponent implements OnInit {
  ...
  setHistoryYearsAndWeeks() {
    ...
    for (let i = 0; i < this.staticDetails.historyYearNumbers.length; i++) {
      ...
      var _yearEndDate = _year.endOf('year');

      for (let j = 0; j < _weeksInYear; j++) {
        var _newWeek = new models.Week();
        _newWeek.startDate = new Date(_yearStartDate.startOf('week').format('L'));
        _newWeek.number = moment(_newWeek.startDate).week();
        _newWeek.serialNumber = j + 1;
  ...
  showChart() {
    ...
    this.historyLineChartData[1].data = [];
    ...
    for (let j = 0; j < this.forecastWeeklyDataByMethod[0].weeks.length; j++) {
      this.historyLineChartData[1].data.push(this.forecastWeeklyDataByMethod[0].weeks[j].units);//blue line
  ...
  validateInputs(): boolean {
    ...
    if(this.selectedHistoryWeek.startDate < this.staticDetails.historyCutOffStartDate){
      alert('History start date cannot be less than ' + this.staticDetails.historyCutOffStartDate.toDateString())
      return false;
    }
    if(this.selectedHistoryWeek.startDate > this.staticDetails.historyCutOffDate){
      alert('History start date cannot be greater than the history cut-off date')
      return false;
    }
    if(this.selectedForecastWeek.startDate < this.staticDetails.historyCutOffDate){
      alert('Forecast start date cannot be earlier than the history cut-off date')
      return false;
    }
    return true;
  }
  ...
  processHistoryData() {
    ...
    for (let i = 0; i < _historySalesDataByWeek.length; i++) {
      ...
      if (_historySalesDataByWeek[i].length > 0) {
        ...
        _newWeek.number = moment(_historySalesDataByWeek[i][0].date).week();
  ...
  processForecastData() {
    ...
    for (let k = 0; k < this.salesDataCollection.forecast.length; k++) {
      ...
      let _groupedForecastWeeks = _.groupBy(this.salesDataCollection.forecast[k].data,
        (result) => moment(result['date']).startOf('week'));
        _.values(_groupedForecastWeeks)
        .forEach(arr => arr.sort((a, b) => moment(a.date).day() - moment(b.date).day()));
  ...
  processMonthlyForecastSummary() {
    ...
    for (let k = 0; k < this.salesDataCollection.forecast.length; k++) {
      ...
      let _groupedForecastMonths = _.groupBy(this.salesDataCollection.forecast[k].data,
        ...
        _.values(_groupedForecastMonths)
        .forEach(arr => arr.sort((a, b) => moment(a.date).day() - moment(b.date).day()));
  ...
  historyPaginatorEvent(event:PageEvent):any  {
  ...
  forecastPaginatorEvent(event:PageEvent):any {
  ...
  setDisplayForecastWeeks(start: number) {
    ...
    for (let i = 0; i < this.displayMethodWeekMap[0].weeks.length; i++) {
      ...
      _test.startDate = moment(this.displayMethodWeekMap[0].weeks[i].startDate).format('L');
  ...
  summaryPaginatorEvent(event:PageEvent):any {
